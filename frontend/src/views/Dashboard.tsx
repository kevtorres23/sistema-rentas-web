...
import { type Incidences, type Inquilino, type VisibleIncidences } from "../types/Incidences";
import {
...
const Dashboard = () => {
    ...
    const incidenciasVisibles: VisibleIncidences[] = useMemo(
        () => dashboardData.dataIncidencias.filter((item: Incidences) => item.name !== 'Por resolver'),
    ...
    useEffect(() => {
        const storedMoraSettings = localStorage.getItem('moraSettings');
        const saved = JSON.parse(storedMoraSettings != null ? storedMoraSettings : "");
        if (saved) setMoraSettings(saved);

        const handleStorage = () => {
            const updated = JSON.parse(storedMoraSettings != null ? storedMoraSettings : "");
    ...
    const datosProcesados = useMemo(() => {
        ...
        let procesados = dashboardData.dataBaseInquilinos.map((item: Inquilino) => {
            let montoFinal = item.montoOriginal;
            let tieneMora = false;
            let mesesRestantes = 10;
            ...
            return { ...item, montoFinal, tieneMora, mesesRestantes };
        });
    ...
    const visibleIncidencesNum = (incidences: VisibleIncidences[]) => {
        const values: number[] = [];

        incidences.forEach((incidence) => {
            values.push(incidence.unknownVal);
        });
                ...
                <div className="row g-3 mb-4">
                    <div className="col-md-3">
                        <div className="h-100 p-3 rounded-4 d-flex flex-column justify-content-center" style={{ backgroundColor: THEME.bgCard }}>
                            ...
                            <div className="fw-bold" style={{ color: THEME.textDark, fontSize: '24px' }}>{formatCurrency(dashboardData.gananciaMensual.toString())}</div>
                        </div>
                    </div>
                    <div className="col-md-3">
                        <div className="h-100 p-3 rounded-4 d-flex flex-column justify-content-center" style={{ backgroundColor: THEME.bgCard }}>
                    ...
                    <div className="col-md-3">
                        <div className="h-100 p-3 rounded-4 d-flex flex-column justify-content-center" style={{ backgroundColor: THEME.bgCard }}>
                            ...
                            <div className="fw-bold" style={{ color: THEME.textDark, fontSize: '24px' }}>{formatCurrency(chargeAmount.toString())}</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="row g-3 mb-4">
                    ...
                    <div className="col-lg-4">
                        <div className="p-4 rounded-4 h-100 d-flex flex-column" style={{ backgroundColor: THEME.bgCard }}>
                            ...
                            <div className="flex-grow-1 position-relative d-flex text-2xl align-items-center justify-content-center">
                                <div style={{ width: '100%', height: 220 }}>
                                    <ResponsiveContainer>
                                        <PieChart>
                                            <Pie data={incidenciasVisibles} innerRadius={65} outerRadius={85} dataKey="value" stroke="none">
                                                {incidenciasVisibles.map((incidencia, index) => (
                                                    <Cell key={`cell-${index}`} fill={incidencia.color} />
                                                ))}
                                                <Label value={visibleIncidencesNum(incidenciasVisibles).reduce((acc, curr) => acc + curr)} position="center" fill={THEME.textDark} style={{ fontSize: '32px', fontWeight: 'bold' }} />
                                            </Pie>
                                        </PieChart>
                                    </ResponsiveContainer>
                                </div>
                ...
                <div className="card w-100 border-0 rounded-4 overflow-hidden" style={{ backgroundColor: THEME.bgCard }}>
                    ...
                    <div className="table-responsive px-2 pb-3">
                        <table className="table w-100 table-borderless align-middle mb-0">
                            ...
                            <tbody>
                                {datosProcesados.length > 0 ? datosProcesados.map((item, id) => (
                                    <tr key={id} style={{ borderBottom: '1px solid #E0E5F2' }}>
                                        ...
                                        <td className="py-3">
                                            <span className="fw-bold" style={{ color: THEME.textDark }}>{formatCurrency(item.montoFinal.toString())}</span>
                                            {item.tieneMora && (
